require 'rails_helper'

RSpec.describe 'Students delete' do
  include ApiHelpers
  include_context "authentication_context"

  let(:api_url) { '/students/:id' }
  let(:student) { FactoryBot.create(:student) }

  describe "DELETE /students/:id", type: :request do
    context 'invalid calls' do
      it 'returns error if user is not logged in' do
        delete api_url.gsub(":id", student.id)

        expect(response).to have_http_status(:forbidden)
      end

      it 'returns not found if student is not found' do
        delete api_url.gsub(":id", "non-existent"), headers: user_headers

        expect(response).to have_http_status(:not_found)
      end
    end

    context 'valid calls' do
      it 'successfully deletes a student' do
        delete api_url.gsub(":id", student.id), headers: user_headers

        expect(response).to have_http_status(:ok)
        expect(Student.exists?(student.id)).to be_falsey
      end
    end
  end
end
